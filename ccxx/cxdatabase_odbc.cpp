...
OdbcColumnDesc emptyOdbcColumnDesc;
...
protected:
    bool openDatabaseImpl(const std::string & sConnectSource,  bool bCreate, const std::map<std::string, std::string> * oParams)
    {
        ...
        {
            try{
                cxDebug()<<"CxDatabaseOdbc->rlogon begin : "<<sConnectSource;
        ...
        cxDebug()<<"CxDatabaseOdbc->rlogon end"<<_db.connected;
...
//        msepoch_t dt0 = 0, dt1 = 0, dt2 = 0, dt3 = 0, dt4 = 0, dt5 = 0, dt6 = 0, dt7 = 0;
        ...
        try
        {
            cxDebug()<<"CxDatabaseOdbc->saveTableImpl begin : "<<connectSource();
...
//        cxDebug()<<"CxDatabaseOdbc->saveTableImpl end "<<_db.connected << ";dt1-dt0=" << dt1-dt0 << ";dt2-dt1=" << dt2-dt1 << ";dt3-dt2=" << dt3-dt2 << ";dt4-dt3=" << dt4-dt3 << ";dt5-dt4=" << dt5-dt4 << ";dt6-dt5=" << dt6-dt5 << ";dt7-dt6=" << dt7-dt6;
        cxDebug()<<"CxDatabaseOdbc->saveTableImpl end "<<_db.connected;
...
//                    _db.commit();
            ...
            {                        // intercept OTL exceptions
                cxDebug()<<"odbc execSqlImpl exception: "<<p.msg<<p.stm_text<<p.sqlstate<<p.var_info<<cxEndLine;
                iResult = FALSE;
            }
            catch(...)
            {
...
//                   "insert into test_tab values(:f1<int>,:f2<varchar_long>)",
        ...
        cxDebug()<<"CxDatabaseOdbc->loadSqlImpl end "<<rows.size();
...
CxDatabaseOdbc * fn_oDatabaseOdbc()
{
    emptyOdbcColumnDesc.columnName.clear();
    emptyOdbcColumnDesc.fsize = 0;
    emptyOdbcColumnDesc.ftype = 0;

    CxDatabaseManager::registDatabaseConstructor(CxDatabaseOdbc::isMyDatabase, CxDatabaseOdbc::createDatabase);

    static CxDatabaseOdbc m;
    return & m;
}

static CxDatabaseOdbc * f_oDatabaseOdbc = fn_oDatabaseOdbc();
