...
bool CxChannelSerial::isSameChannelImpl(const std::map<std::string, std::string> &params) const
{
    ...
    int iBaudRate = iDefault;
    int iDataBits = iDefault;
    int iStopBits = iDefault;
    int iParity   = iDefault;
    int iFlow     = iDefault;
    CxString::fromString(CxContainer::value( params, CS_EntryBaudRate, sDefault), iBaudRate);
    CxString::fromString(CxContainer::value( params, CS_EntryCharacterSize, sDefault), iDataBits);
    CxString::fromString(CxContainer::value( params, CS_EntryStopBits, sDefault), iStopBits);
    CxString::fromString(CxContainer::value( params, CS_EntryParity  , sDefault), iParity  );
    CxString::fromString(CxContainer::value( params, CS_EntryFlowControl    , sDefault), iFlow    );
...
void CxChannelSerial::ReceiverThread::run()
{
    ...
    OVERLAPPED olEvent;
    memset(&olEvent, 0, sizeof(OVERLAPPED));
    olEvent.hEvent = CreateEvent(NULL, FALSE, FALSE, NULL);

    HANDLE HandlesToWaitFor[2];
    HandlesToWaitFor[0] = olRead.hEvent;
    HandlesToWaitFor[1] = olEvent.hEvent;

    if (! ::SetCommMask(hFd, EV_ERR or EV_RLSD or EV_RING ))
    {
        CloseHandle(olRead.hEvent);
        CloseHandle(olEvent.hEvent);
        return;
    }

    DWORD lpfdwEvtMask;
    WaitCommEvent( hFd, &lpfdwEvtMask, &olEvent );

    if(::ReadFile(hFd, _buffer, sizeof(_buffer), &dwLength, &olRead) == FALSE)
    {
        if(GetLastError() == ERROR_IO_PENDING)
        {
            WaitForSingleObject(olRead.hEvent, INFINITE);
            GetOverlappedResult(hFd, &olRead, &dwLength, TRUE);
        }
        else
        {
            ClearCommError(hFd, &dwError, &cs);
    ...
    do
    {
        DWORD dwHandleSignaled = WaitForMultipleObjects(2, HandlesToWaitFor, false, INFINITE);
