...
void fn_void_pchar(pid_os_t pid, const std::string & sArgs)
{
    if (f_win)
    {
        f_win->outInfo(QString::fromUtf8("进程已经退出[%1]").arg(pid.dwProcessId));
        f_win->show();
    }
}
    ...
    ui(new Ui::TestChannelWin)
{
    ...
    CxProcess::registProcessNotify(fn_void_pchar);
}
